    ...
    <div className="min-h-screen px-4 sm:px-6 py-6 max-w-6xl mx-auto">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-heading" style={{ color: '#1A1A2E' }}>Meu Universo</h1>
            <p className="font-body text-gray-500 text-sm">As incríveis criações de {user.name}</p>
          </div>
        </div>

        <Link href="/builder">
          <Button className="hidden sm:flex h-11">
      ...
      <div className="grid grid-cols-3 gap-3 mb-6">
        {[
          { label: 'Criações', value: creations.length, emoji: '🏗️', color: '#7B2D8B' },
          { label: 'Total de Peças', value: totalBricksUsed, emoji: '🧱', color: '#D01012' },
          { label: 'Total de Curtidas', value: totalLikes, emoji: '❤️', color: '#D01012' },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
            initial={{ opacity: 0, y: 20 }}
          ...
          >
            ...
            <div className="text-xl sm:text-2xl font-heading" style={{ color: stat.color }}>
      ...
      >
        <div className="-mx-4 px-4 flex items-center gap-1.5 overflow-x-auto pb-1">
          <SortAsc className="w-4 h-4 text-gray-400 flex-shrink-0" />
          <p className="text-sm font-body font-semibold text-gray-400 mr-1 flex-shrink-0">Ordenar:</p>
          {(['newest', 'likes', 'bricks'] as const).map((sort) => (
            <motion.button
              key={sort}
              onClick={() => setSortBy(sort)}
              className="flex-shrink-0 px-3 py-1.5 rounded-xl text-xs font-bold font-body border-2 capitalize transition-all"
              style={{
              ...
              whileTap={{ scale: 0.95 }}
            >
              {sort === 'newest' ? '🕒 Recentes' : sort === 'likes' ? '❤️ Curtidas' : '🧱 Mais Peças'}
            </motion.button>
          ))}
        </div>

        <div className="flex-shrink-0 flex items-center gap-1 bg-gray-100 rounded-xl p-1 ml-2">
          {(['grid', 'list'] as const).map((mode) => (
            <button
              key={mode}
              onClick={() => setViewMode(mode)}
              className="p-1.5 rounded-lg transition-all"
              aria-label={mode === 'grid' ? 'Modo grade' : 'Modo lista'}
              style={{
      ...
      {sorted.length > 0 ? (
        <div className={viewMode === 'grid'
          ...
          <Link href="/builder">
            <motion.div
              ...
              className="h-56 border-4 border-dashed rounded-3xl flex flex-col items-center justify-center gap-3 cursor-pointer transition-all"
              style={{ borderColor: '#7B2D8B40', color: '#7B2D8B' }}
              whileHover={{ scale: 1.02, borderColor: '#7B2D8B' }}
            >
              <span className="text-5xl">+</span>
              <span className="font-heading text-lg">Criar Algo Incrível!</span>
              <span className="font-body text-sm opacity-70">Abrir o Construtor 3D</span>
            </motion.div>
          </Link>
        </div>
      ) : (
        ...
        >
          ...
          <h3 className="text-2xl font-heading mb-2" style={{ color: '#1A1A2E' }}>Nenhuma Criação Ainda!</h3>
          <p className="font-body text-gray-500 mb-6">Comece a construir algo incrível no Construtor 3D!</p>
          <Link href="/builder">
